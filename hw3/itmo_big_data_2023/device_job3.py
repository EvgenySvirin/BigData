...
def python_data_stream_example():
    ...
    def max_temperature(l, r):
        return r if l.temperature < r.temperature else l

    (ds.window_all(
        TumblingProcessingTimeWindows.of(Time.seconds(5)))
     .reduce(max_temperature)
     .map(TemperatureFunction(), Types.STRING()).sink_to(sink))

    env.execute_async("Tumbling windows")
